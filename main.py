...
db=client["ocrtestdb"]
doc_collection=db["doccollection"]
...
def convert_and_save_pdf2image(files):
    ...
    for file in files:
        if (file.filename[-4:]==".pdf"):
            ...
            doc_id = hashlib.sha256(save_path.encode('utf-8')).hexdigest()
            ...
            for idx, page in enumerate(pages):
                ...
                img_id = hashlib.sha256(save_path.encode('utf-8')).hexdigest()

                doc_collection.insert_one({"doc_id":doc_id, "img_id":img_id})
            ...
            doc_collection.insert_one({"doc_id":img_id, "img_id":img_id})
...
async def paddleocr(imgs: List[UploadFile] = File(...)):
    ...
    for img in imgs:
        ...
        for line in result:
            text+=" "+line[-1][0]
        id = hashlib.sha256(img.encode('utf-8')).hexdigest()

        if (image_collection.find_one({"_id":id})):
            print("Transcription already exists with id: {}".format(id))
        else:
            image_collection.insert_one({"_id":id, "transcription":text})
...
async def search( request: Request, query):
    results = image_collection.find({'$text':{'$search':query}})  
...
#                 <h2>c) prepare_raw_transcription from paddleocr</h2>
